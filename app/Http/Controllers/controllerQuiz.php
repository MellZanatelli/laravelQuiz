...
class controllerQuiz extends Controller
{
    ...
    public function dadosPagina5(Request $request){
        ...
        if( $acertos == 5) {
            $dados['mensagem'] = "PARABÉNS! As bactérias estão orgulhosas de você <3 
            Você obteve ".$acertos." acertos.";
        } else if ( $acertos >= 3){
            $dados['mensagem'] = "JÁ E MAIS DA METADE... Um pouquinho mais de estudo e você chega lá! 
            Você obteve ".$acertos." acertos.";
        } else if ( $acertos >= 1){
            $dados['mensagem'] = "ESTÁ PRECISANDO MELHORAR! Aprenda sobre os microrganismos, é importante! 
            Você obteve ".$acertos." acertos.";
        } else if ( $acertos == 0){
            $dados['mensagem'] = "VOCÊ NÃO PASSA DESTA NOITE! As bactérias estão prontas para te devorar... 
            Você obteve ".$acertos." acertos.";
        }
